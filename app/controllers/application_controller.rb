class ApplicationController < ActionController::Base
  ...
  def db_reset
    ...
    (comment = Post.new({:post => post_map[1], :content => "Agree", :user => user_map[1], :title => "Re:Project Due"})).save()
    Post.new({:post => comment, :content => "Nested Comment", :user => user_map[1], :title => "Re:Re:Project Due"}).save()
    (vote_map = {
